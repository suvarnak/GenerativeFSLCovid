"""
 custom Data loader, as given in  tutorial
"""
import imageio
import torch
...
class CustomDataset(Dataset):
    ...
    def __init__(self, name, base_path, transform=None,random_shots=10):
        #super(Dataset, self).__init__()
        self.base_path = base_path
        self.inputFolderDataset = datasets.ImageFolder(root=self.base_path + '/')
        self.transform = transform
        self.to_tensor = transforms.ToTensor()
        self.random_shots = random_shots

    def __len__(self):
        return self.random_shots
...
class TargetRandomShotDataLoader():
    def __init__(self, config):
        ...
        if config.data_mode == "imgs":
            ...
            print("@@@@@@@@",len(target_dataset_train))
            random_shots_indices = random.sample(range(0, len(target_dataset_train)),self.config.no_of_shots )
            print("@@@@@@@@",random_shots_indices)
            target_dataset_train_subset = torch.utils.data.Subset(target_dataset_train, random_shots_indices)
            print("@@@@@@@@",len(target_dataset_train_subset))
            self.train_loader = torch.utils.data.DataLoader(
                target_dataset_train_subset, batch_size=self.config.batch_size, shuffle=True, num_workers=self.config.data_loader_workers, pin_memory=self.config.pin_memory)
