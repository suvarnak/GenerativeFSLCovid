...
class BalancedFSLDiscriminatorAgent(BaseAgent):
    def __init__(self, config):
        ...
        # define optimizer
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.config.learning_rate, eps=1e-08, weight_decay=0, betas=(0.9, 0.999), amsgrad=False)
        
        #optim.RMSprop(self.model.parameters(
    ...
    def save_checkpoint(self, filename='source_model_checkpoint.pth.tar', is_best=0):
        ...
        shutil.copyfile(self.config.checkpoint_dir + filename,
                        self.config.checkpoint_dir + str(state['epoch']) + filename)
    ...
    def finetune_model(self, domain_name):
        ...
        try:
            if self.load_source_model():
                self.data_loader = BalancedTargetDataLoader(config=self.config)
